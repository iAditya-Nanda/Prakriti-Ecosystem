...
const AIChatThreadScreen = ({ route, navigation }) => {
  ...
  const ask = async (text) => {
    ...
    // Inject the bot message bubble immediately so text flows into it in real time
    setMessages((prev) => [...prev, botMsg]);
    scrollToBottom();

    const handleChatError = (errDetail) => {
    ...
    try {
      ...
      // Hide the typing spinner as soon as the stream starts delivering data
      setShowTyping(false);
      setLoadingReply(false);
          ...
          if (trimmed.startsWith("data: ")) {
            try {
              ...
              if (data.chunk) {
                ...
                accumulatedText = `⚠️ **Generation Error**: ${data.error}`;
                updatedText = true;
              }
            } catch (e) {
      ...
      // Persist final chat state in local storage
      setMessages((prev) => {
        saveHistory(prev);
        return prev;
      });
      ...
      console.log("Chat streaming error:", e);
      handleChatError(e.message || "Streaming Connection Failed");
    }
  };
